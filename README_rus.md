...
**`deadline`** ⏤ время окончания приёма вступительных взносов в формате Unix-времени. Первоначальное значение переменной `entryEndTime` задаётся при исполнении конструктора контракта. Впоследствии оно может быть изменено, но только один раз.
...
**`entriesCounters`** ⏤ соответствие <адрес участника> => <счётчик числа входов участника>.
...
**`onLeaving`** ⏤ генерируется после каждого выхода участника из турнира (см. описание функции `leave`). С данным событием передаются следующие параметры:
...
**`onTermination`** ⏤ генерируется после присвоения турниру одного из статусов *LackOfPlayers*, *Cancelled* или *NoWinner*. Статус *LackOfPlayers* может присваиваться в функцияи `withdraw`, статусы *Cancelled* и *NoWinner* ⏤ в функции `terminate`. С данным событием передаётся новый статус турнира.
