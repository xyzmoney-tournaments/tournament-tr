...
При этом в переменную `organizer` заносится адрес создателя контракта, а остальные переменные инициализируются значениями соответствующих входных аргументов конструктора. В дальнейшем время окончания приёма вступительных взносов может быть изменено организатором турнира только один раз, остальные переменные в дальнейшем не меняются.
...
Если после истечения времени приёма вступительных взносов число игроков окажется не меньше минимального количества игроков, турнир считается начавшимся. Чтобы завершить турнир, организатор должен или объявить победителя турнира посредством вызова функции `announceWinner` (в результате чего турниру автоматически присваивается статус *Winner*), или объявить турнир завершённым без победителя, вызвав функцию `terminate` с явным присвоением турниру одного из статусов *Cancelled* или *NoWinner*. Выбор статуса определяет организатор турнира в зависимости от причины отсутствия победителя. Завершить турнир путём объявления победителя можно не ранее чем через 19 часов после истечения времени приёма вступительных взносов. В противовес этому, завершить турнир без победителя можно ещё до окончания приёма вступительных взносов.
...
**`onCreation`** ⏤ вызывается в ходе выполнения конструктора контракта (см. описание функции `constructor`). С данным событием передаются следующие аргументы:
* минимальное количество игроков;
* размер вступительного взноса;
* доля победителя;
* время окончания приёма вступительных взносов.

**`onEtnrance`** ⏤ вызывается после каждого входа участника в турнир (см. описание функции `enter`). С данным событием передаются следующие аргументы:
* адрес вошедшего участника;
* входной код, введенный данным участником;
* счётчик числа входов этого участника после данного входа.

**`onUnregistering`** ⏤ вызывается после каждого выхода участника из турнира (см. описание функции `unregister`). С данным событием передаются следующие аргументы:
...
**`onPrizePayment`** ⏤ вызывается после того, как победитель забрал свой приз (см. описание функции `takePrize`). С данным событием передаются следующие аргументы:
...
Конструктор исполняется автоматически при создании контракта. Он сохраняет адрес создателя контракта в переменной `organizer`, а значения входных аргументов ⏤ в одноимённых переменных, после чего вызывает событие `onCreation`, публикующее указанные значения входных параметров.
...
После успешного прохождения входных условий функция меняет значение переменной `deadline` на значение входного аргумента, присваивает переменной `deadlineIsChanged` значение *true* и вызывает событие `onDeadlineChange`, публикующее новое значение времени окончания приёма вступительных взносов.
...
* победитель может быть объявлен не ранее чем через 19 часов после завершения приёма вступительных взносов;
* победитель может быть только из числа игроков.

После успешного прохождения входных условий функция обновляет остаток средств, доступный организатору для снятия (переменная `availableFunds`). Далее функция заносит в переменную `winner` значение входного аргумента, а в переменную `status` ⏤ статус *Winner*, затем вызывает событие `onWinnerAnnouncement`, публикующее адрес победителя.
...
После успешного прохождения входных условий функция заносит в переменную `status` значение входного аргумента, а затем вызывает событие `onTermination`, публикующее новый статус турнира.
